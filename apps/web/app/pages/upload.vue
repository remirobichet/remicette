...
async function handleSubmit() {
  ...
  try {
    await $fetch(`${config.public.apiUrl}/recipes`, {
      method: "POST",
      body: { url: url.value, code: code.value },
    });
    success.value = "URL soumise avec succès !";
  } catch (e) {
    console.error(e);
    ...
    <Card class="w-full max-w-lg">
      ...
      <CardContent class="space-y-4">
        <form class="space-y-3" @submit.prevent="handleSubmit">
          ...
          <Input v-model="code" placeholder="Password" type="password" />
          ...
          >
            ...
            <Icon v-else class="animate-spin h-4 w-4 mx-auto" name="loader" />
          </Button>
        </form>
